...
Future<void> pumpTerminalView(WidgetTester tester, {Size size = const Size(800, 600)}) async {
...
void main() {
  testWidgets('Control characters should not be visible', (tester) async {
    ...
    print('Buffer text runes: ${bufferText.runes.map((r) => r.toRadixString(16)).join(" ")}');
  ...
  testWidgets('SGR 8 invisible flag hides text and underlines', (tester) async {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      print('Test cell $i: underlineStyle=${cell.underlineStyle}, flags=0x${cell.flags.toRadixString(16)}, content=0x${(cell.content & CellContent.codepointMask).toRadixString(16)}');
      // With invisible flag, the foreground should not be painted
      // But underlineStyle is still stored
      expect(cell.flags & CellFlags.invisible, isNot(equals(0)), reason: 'Cell $i should have invisible flag');
    }
  });
}
